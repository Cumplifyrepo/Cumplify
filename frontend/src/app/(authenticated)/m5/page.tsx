...
/**
 * M5 Risk Management — view-designs.md §9.
 * Register: getCrossRegisterRiskView(standard, category) as the list query.
 * Standard pills filter + category select filter.
 * Row expand: treatments. createRisk, addRiskTreatment, createChangePlan drawers.
 * Real-time: onRiskEscalated → refetch + flash the escalated row.
 * Ask chip target: consumes ?create=1&title=<>&standard=<> from URL params.
 */
...
export default function M5RiskManagementPage() {
  ...
  // Ask chip: consumes ?create=1&title=<>&standard=<>
  ...
  }, [query, filterStandard, filterCategory]);
