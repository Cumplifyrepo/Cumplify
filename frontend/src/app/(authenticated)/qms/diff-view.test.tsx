...
const mockDiffContent = JSON.stringify({
  '4.1-context': { added: ['New sentence about external issues.'], removed: ['Old sentence about context.'], kindChange: null },
...
describe('DiffView — content rendering', () => {
  ...
  it('renders per-section diff with added and removed lines', async () => {
    ...
    expect(screen.getByTestId('diff-section-4.1-context').textContent).toContain('New sentence about external issues');
    expect(screen.getByTestId('diff-section-4.1-context').textContent).toContain('Old sentence about context');
  });
  ...
  it('renders kindChange label when section type changed', async () => {
    ...
    expect(screen.getByTestId('diff-section-6.1-risks').textContent).toContain('Section type changed');
  });
});
