...
vi.mock('next-intl', () => {
  const t: Record<string, Record<string, string>> = {
    'qms.docViewer': {
      title: 'Document Viewer', versions: 'Version History', diffTitle: 'Version Diff',
      diffUnavailable: 'Diff unavailable for this version', noVersions: 'Single version — no diff available',
      additions: 'additions', deletions: 'deletions', kindChange: 'Section type changed',
      selectVersions: 'Select two versions to compare', back: 'Back',
      unresolvedGaps: 'Unresolved gap sections remain',
      sodViolation: 'Segregation of duties violation: approver cannot be the author',
      export: 'Export', exportBlocked: 'Export not yet available (awaiting backend)',
      submitForApproval: 'Submit for approval',
      unreviewedSections: 'Not all sections have been reviewed. Review all prose sections before submitting.',
      submitSuccess: 'Document submitted for approval successfully.',
      versionLabel: 'Version {version}', selectVersionForDiff: 'Select version {version} for diff comparison',
    },
    'qms.generation': {
      gapSection: 'GAP', failedSection: 'Failed', naSection: 'N/A',
      proseSection: 'Content ready', reviewed: 'Reviewed', unreviewed: 'Not reviewed',
...
const mockVersions = [
  { id: 'v1', documentId: 'doc-1', versionNo: 1, contentRef: 's3://bucket/key', changeSummary: 'Initial', authorId: 'u1', createdAt: '2026-07-15T09:00:00Z' },
];
...
const mockContent = JSON.stringify({
  ...
  locale: 'en',
  frontMatter: {
    purpose: 'This manual was generated from the organization\u2019s own recorded data. Where required information was absent, the affected section is shown as an explicit gap.',
    scope: { organization: 'Acme Corp', standards: ['ISO9001', 'ISO14001'], sites: ['HQ, City, Country'], managementRepresentative: 'Jane' },
    normativeRefs: [{ standard: 'ISO9001', source: 'https://www.iso.org/store.html' }, { standard: 'ISO14001', source: 'https://www.iso.org/store.html' }],
    terms: [],
  },
  sections: [
    { harmonizationKey: '4.1-context', clauseRefs: [{ standard: 'ISO9001', clauseNo: '4.1' }], kind: 'prose', sentences: [{ text: 'The organization has determined external and internal issues.', factRefs: ['F1'] }] },
    { harmonizationKey: '6.1-risks', clauseRefs: [{ standard: 'ISO9001', clauseNo: '6.1' }], kind: 'gap', gap: { missingSources: ['register.risk_assessments', 'register.aspects_register'] } },
    { harmonizationKey: '7.2-competence', clauseRefs: [{ standard: 'ISO9001', clauseNo: '7.2' }], kind: 'na_justified', naJustification: 'Design not in scope' },
    { harmonizationKey: '8.1-ops', clauseRefs: [{ standard: 'ISO9001', clauseNo: '8.1' }], kind: 'failed', failed: true },
  ],
});
...
const mockRunSections = [
  ...
  { id: 's3', harmonizationKey: '7.2-competence', kind: 'NA_JUSTIFIED', reviewedBy: null, reviewedAt: null },
...
beforeEach(() => {
  ...
  mockQuery.mockImplementation((q: string) => {
    if (q.includes('listDocumentVersions')) return Promise.resolve({ listDocumentVersions: mockVersions });
    if (q.includes('getDocumentContent')) return Promise.resolve({ getDocumentContent: mockContent });
    if (q.includes('listGenerationRuns')) return Promise.resolve({ listGenerationRuns: [{ manualDocumentId: 'doc-1', sections: mockRunSections }] });
...
describe('DocumentViewer — content rendering (derive.ts contract)', () => {
  it('renders BC-1 disclaimer from frontMatter.purpose', async () => {
    ...
    expect(screen.getByTestId('bc1-disclaimer').textContent).toContain('generated from the organization');
  });
  ...
  it('renders ISO store link from normativeRefs[0].source', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('bc1-disclaimer')).toBeInTheDocument());
    expect(screen.getByTestId('bc1-disclaimer').querySelector('a')?.href).toBe('https://www.iso.org/store.html');
  });
  ...
  it('renders prose sections with sentence text', async () => {
    ...
    expect(screen.getByTestId('doc-section-4.1-context').textContent).toContain('external and internal issues');
  });
  ...
  it('renders GAP blocks with missingSources (no sentences, visually distinct)', async () => {
    ...
    expect(screen.getByTestId('gap-block-6.1-risks').textContent).toContain('register.risk_assessments');
    expect(screen.getByTestId('gap-block-6.1-risks').textContent).toContain('register.aspects_register');
  });

  it('renders N/A justified sections with justification text', async () => {
    render(<DocumentViewer documentId="doc-1" onBack={mockOnBack} onDiff={mockOnDiff} />);

    await waitFor(() => expect(screen.getByTestId('doc-section-7.2-competence')).toBeInTheDocument());
    expect(screen.getByTestId('doc-section-7.2-competence').textContent).toContain('Design not in scope');
  });
...
describe('DocumentViewer — review action (joined from run.sections)', () => {
  it('shows review button for prose sections when role can approve M1 and runSection exists', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('doc-review-btn-4.1-context')).toBeInTheDocument());
  ...
  it('calls markSectionReviewed with sectionId from the run section (not content)', async () => {
    mockMutate.mockResolvedValue({ markSectionReviewed: { id: 's1', reviewedBy: 'u1', reviewedAt: '2026-07-15T12:00:00Z' } });
    render(<DocumentViewer documentId="doc-1" onBack={mockOnBack} onDiff={mockOnDiff} />);
...
describe('DocumentViewer — submit for approval errors', () => {
  it('surfaces UNREVIEWED_SECTIONS with proper i18n message', async () => {
    ...
    expect(screen.getByTestId('submit-error').textContent).toContain('Not all sections have been reviewed');
  });
  ...
  it('shows success message on successful submit', async () => {
    mockMutate.mockResolvedValue({ submitDocumentForApproval: { id: 'doc-1', status: 'IN_REVIEW' } });
    render(<DocumentViewer documentId="doc-1" onBack={mockOnBack} onDiff={mockOnDiff} />);
    ...
    expect(screen.getByTestId('submit-success').textContent).toContain('submitted for approval successfully');
  });
});
