...
vi.mock('next-intl', () => {
  const translations: Record<string, Record<string, string>> = {
    ...
    'forms.catalog': { title: 'Form Templates', filterAll: 'All', loading: 'Loading...', sections: 'sections', fields: 'fields' },
    'forms.register': { colStatus: 'Status', colCompletion: 'Completion', colOpenedBy: 'Opened', colUpdated: 'Updated', newRecord: 'New', backToCatalog: 'Back', loading: 'Loading...', empty: 'No records' },
    'forms.form': { loading: 'Loading...', back: 'Back', submit: 'Submit', approve: 'Approve', reopen: 'Reopen' },
  };
  return {
...
vi.mock('@/components/shared', () => ({
  ...
  DataTable: ({ data, columns }: { data: unknown[]; columns: { key: string; render: (item: unknown) => React.ReactNode }[] }) => (
    <table data-testid="data-table"><tbody>{data.map((item, i) => <tr key={i}>{columns.map(c => <td key={c.key}>{c.render(item)}</td>)}</tr>)}</tbody></table>
  ),
...
const mockTemplates = [
  { id: 'tpl-1', key: 'ncr', titleKey: 'forms.tpl.ncr.title', descriptionKey: 'forms.tpl.ncr.desc', category: 'corrective', clauseRefs: ['8.7', '10.2'], standards: ['ISO9001', 'ISO14001', 'ISO45001', 'IMS'], requiresApproval: true, sectionCount: 6, fieldCount: 35 },
  { id: 'tpl-2', key: 'hira', titleKey: 'forms.tpl.hira.title', descriptionKey: 'forms.tpl.hira.desc', category: 'ohs', clauseRefs: ['6.1.2'], standards: ['ISO45001', 'IMS'], requiresApproval: false, sectionCount: 3, fieldCount: 12 },
];
