'use client';

import { useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { useTranslations } from 'next-intl';

/**
 * Forms catalog moves here in P3; the working spec-41 forms engine serves until absorption (architecture §11).
 * Interim redirect: the new IA route must always land on a WORKING surface —
 * redirect direction flips only when the absorbing view ships its design gate.
 */
export default function FormsInterimRedirect() {
  const router = useRouter();
  const t = useTranslations('nav');

  useEffect(() => {
    router.replace('/m4/forms');
  }, [router]);

  return (
    <p style={{ color: 'var(--color-text-muted)', padding: 'var(--space-xl)' }}>
      {t('redirecting')}
    </p>
  );
}
