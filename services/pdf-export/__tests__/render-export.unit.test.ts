...
describe('PdfRenderFn guards + cache', () => {
  it('assertTenantKey rejects keys outside the tenant prefix', () => {
    ...
    expect(() => assertTenantKey(T, 'tenants/tenant-other/documents/d/v1.json')).toThrow(
      'TENANT_KEY_MISMATCH',
    );
        ...
        documents: [
          {
            documentId: 'd',
            versionId: 'v',
            contentKey: 'tenants/tenant-other/x.json',
            title: 't',
            docType: 'manual',
            standard: 'IMS',
            versionNo: 1,
...
describe('ExportFn', () => {
  ...
  const masterEntries = [
    {
      ...
      standard: 'IMS',
      versionNo: 1,
      contentRef: `tenants/${T}/documents/doc-manual/v1.json`,
    },
    {
      documentId: 'doc-clause',
      title: 'Context (4.1)',
      docType: 'procedure',
      standard: 'ISO9001',
      versionNo: 1,
      contentRef: `tenants/${T}/documents/doc-clause/v1.json`,
    },
  ];
  const candidates = [
    {
      documentId: 'ml-other',
      versionId: 'ver-ml-o',
      contentKey: `tenants/${T}/documents/ml-other/v1.json`,
      title: 'Master List (other run)',
      standard: 'IMS',
      versionNo: 1,
    },
    {
      documentId: 'ml-match',
      versionId: 'ver-ml-m',
      contentKey: `tenants/${T}/documents/ml-match/v1.json`,
  ...
  function wireHappyPath() {
    mockS3Send.mockImplementation(
      (cmd: { constructor: { name: string }; input: { Key?: string } }) => {
        const key = cmd.input.Key ?? '';
        if (cmd.constructor.name === 'GetObjectCommand') {
          if (key.includes('ml-other'))
            return Promise.resolve(s3Json({ entries: [{ documentId: 'someone-else' }] }));
          ...
          results: [
            {
              documentId: 'doc-manual',
              versionId: 'ver-manual',
              pdfKey: `tenants/${T}/pdf/doc-manual-a.pdf`,
              sha256: 'a',
              cached: false,
            },
            {
              documentId: 'doc-clause',
              versionId: 'doc-clause-v1',
              pdfKey: `tenants/${T}/pdf/doc-clause-b.pdf`,
              sha256: 'b',
              cached: true,
            },
            {
              documentId: 'ml-match',
              versionId: 'ver-ml-m',
              pdfKey: `tenants/${T}/pdf/ml-match-c.pdf`,
              sha256: 'c',
              cached: true,
            },
          ],
  ...
  it('resolves the export set from the master list that CONTAINS the manual (skips non-matching candidate)', async () => {
    ...
    expect(invokePayload.documents.map((d: { documentId: string }) => d.documentId)).toEqual([
      'doc-manual',
      'doc-clause',
      'ml-match',
    ]);
  ...
  it('uploads a real ZIP (PK header) with the manual as 00- first entry, presigned at 900s', async () => {
    ...
    const put = mockS3Send.mock.calls.find(
      (c) =>
  ...
  it('EXPORT_SET_NOT_FOUND when no master list contains the manual', async () => {
    mockS3Send.mockImplementation(() =>
      Promise.resolve(s3Json({ entries: [{ documentId: 'x' }] })),
    );
    await expect(
      exportHandler({ tenantId: T, manual, masterListCandidates: candidates }),
  ...
  it('RENDER_FAILED when PdfRenderFn returns FunctionError', async () => {
    mockS3Send.mockImplementation((cmd: { input: { Key?: string } }) =>
      Promise.resolve(
        String(cmd.input.Key).includes('ml-match')
