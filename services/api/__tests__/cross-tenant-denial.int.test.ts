...
const CLUSTER_ARN = process.env.C7_CLUSTER_ARN ??
  'arn:aws:rds:us-east-1:697114252993:cluster:dev-datastack-auroracluster23d869c0-zmmgimmc0vnd';
const APP_ROLE_SECRET = process.env.C7_APP_ROLE_SECRET_ARN ??
  'arn:aws:secretsmanager:us-east-1:697114252993:secret:cumplify/dev/rds/app-role-3cYWNR';
const TENANT_DATA_ROLE = process.env.C7_TENANT_DATA_ROLE_ARN ??
  'arn:aws:iam::697114252993:role/cumplify-dev-tenant-data-role';
const TABLE_ARN = process.env.C7_TABLE_ARN ??
  'arn:aws:dynamodb:us-east-1:697114252993:table/CumplifyCore';
...
function rlsQuery(tenant: string | null, sql: string): unknown[] {
  const tx = JSON.parse(
    aws(`rds-data begin-transaction --resource-arn ${CLUSTER_ARN} --secret-arn ${APP_ROLE_SECRET} --database ${DB_NAME}`),
  ...
  try {
    ...
    try {
      aws(`rds-data rollback-transaction --resource-arn ${CLUSTER_ARN} --secret-arn ${APP_ROLE_SECRET} --transaction-id ${tx}`);
    } catch { /* best effort */ }
  }
}
...
describe.skipIf(!LIVE)('C-7 #2: DynamoDB cross-tenant denial (simulate-principal-policy)', () => {
  function simulate(tagTenant: string, keyPrefix: string): string {
    const ctx = JSON.stringify([
      { ContextKeyName: 'aws:PrincipalTag/tenantId', ContextKeyType: 'string', ContextKeyValues: [tagTenant] },
      { ContextKeyName: 'dynamodb:LeadingKeys', ContextKeyType: 'string', ContextKeyValues: [`${keyPrefix}#DOC#1`] },
    ]);
    const out = JSON.parse(
      aws(
...
describe.skipIf(!process.env.C7_POOL_B_TOKEN)('C-7 #6: subscription cross-tenant denial (C-6)', () => {
  const GQL_URL = process.env.C7_GRAPHQL_URL ??
  ...
  function subscribeCrossTenant(token: string, argTenant: string): Promise<string> {
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      const timer = setTimeout(() => { try { ws.close(); } catch { /**/ } reject(new Error('timeout')); }, 25_000);
      const done = (r: string) => { clearTimeout(timer); try { ws.close(); } catch { /**/ } resolve(r); };
      ...
      ws.onmessage = (ev: MessageEvent) => {
        ...
        if (msg.type === 'connection_ack') {
          ...
          ws.send(JSON.stringify({
            id: 'c6-denial', type: 'start',
            payload: { data: JSON.stringify({ query, variables: {} }), extensions: { authorization: authHeader } },
          }));
      ...
      ws.onerror = () => { clearTimeout(timer); reject(new Error('ws transport error')); };
    });
  }
