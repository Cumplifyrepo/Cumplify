...
export const RECORDS_VAULT_TOOLS: ToolConfig[] = [
  {
    toolSpec: {
      name: 'records-retain',
      description: 'Trigger record sealing for a completed record. Advisory — no HITL required (immutability layer).',
      inputSchema: {
        json: {
          type: 'object',
          ...
          properties: {
            ...
            retentionCategory: { type: 'string', description: 'Retention category: permanent | 7-year | 5-year | 3-year' },
  ...
  },
  {
    toolSpec: {
      ...
      description: 'Append an audit trail entry to a sealed record. Advisory — no HITL required (immutable append-only).',
      inputSchema: {
        json: {
          type: 'object',
          ...
          properties: {
            ...
            action: { type: 'string', description: 'Action type: access | review | disposition-check' },
  ...
  },
  {
    toolSpec: {
      ...
      description: 'Modify a retention schedule for a record category. HITL-gated: requires human approval.',
      inputSchema: {
        json: {
          type: 'object',
          ...
          properties: {
            ...
            retentionPeriod: { type: 'string', description: 'New retention period (e.g., 7-year, permanent)' },
