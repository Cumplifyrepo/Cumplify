...
function docContentKey(tenantId: string, documentId: string): string {
  return `tenants/${tenantId}/documents/${documentId}/v1.json`;
}
...
async function writeVersion(
  ...
  await txn.execute(
    `INSERT INTO m1.document_versions
     ...
     VALUES (:tenantId, :docId::uuid, 1, :contentRef, :contentSha, 'Generated by DocGen run', :owner, :owner)`,
    [
      { name: 'tenantId', value: { stringValue: tenantId } },
...
export async function handler(event: FinalizeInput): Promise<FinalizeOutput> {
  ...
  try {
    ...
    // 1. Manual
    manualDocumentId = await insertDocument(txn, tenantId, {
