...
export function buildProfileFacts(profile: Record<string, unknown>): Omit<Fact, 'key'>[] {
  ...
  if (present(profile.legalName))
    add('legalName', `The organization's legal name is ${profile.legalName}.`);
  if (present(profile.industry))
    add('industry', `The organization operates in the ${profile.industry} industry.`);
  if (present(profile.productsServices))
    add('productsServices', `Products and services: ${profile.productsServices}.`);
  if (present(profile.employeeCount))
    add('employeeCount', `The organization employs ${profile.employeeCount} people.`);
  if (present(profile.managementRep))
    add('managementRep', `The named management representative is ${profile.managementRep}.`);
  if (present(profile.yearFounded))
    add('yearFounded', `The organization was founded in ${profile.yearFounded}.`);
  if (present(profile.supplyChainShape))
    add('supplyChainShape', `Supply-chain shape: ${profile.supplyChainShape}.`);
  if (present(profile.targetCertDate))
    add('targetCertDate', `Target certification date: ${profile.targetCertDate}.`);
  if (profile.designResponsibility !== undefined) {
    add(
      'designResponsibility',
      profile.designResponsibility
  ...
  if (present(profile.outsourcedProcesses)) {
    add(
      'outsourcedProcesses',
      `Outsourced processes: ${(profile.outsourcedProcesses as string[]).join(', ')}.`,
    );
  }
  if (present(profile.existingCertifications)) {
    add(
      'existingCertifications',
      `Existing certifications: ${(profile.existingCertifications as string[]).join(', ')}.`,
    );
  }
  if (present(profile.standardsInScope)) {
    add(
      'standardsInScope',
      `Standards in scope: ${(profile.standardsInScope as string[]).join(', ')}.`,
    );
  }
  if (Array.isArray(profile.sites)) {
    (profile.sites as SiteShape[]).forEach((site, i) => {
      if (!present(site?.name)) return;
      const locality = [site.address, site.city, site.state, site.country]
        .filter(Boolean)
        .join(', ');
      const headcount = present(site.headcount) ? ` (headcount ${site.headcount})` : '';
      add(
        `sites.${i}`,
        `The organization operates a site "${site.name}"${locality ? ` at ${locality}` : ''}${headcount}.`,
      );
    });
  }
  return facts;
...
export function assembleFacts(profile: Record<string, unknown>, registers: RegisterData[]): Fact[] {
  const unnumbered = [...buildProfileFacts(profile), ...registers.flatMap(buildRegisterFacts)];
