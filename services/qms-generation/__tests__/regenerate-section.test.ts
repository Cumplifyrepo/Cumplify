...
const { mockExecute, mockCommit, mockRollback, mockPublishAudit, mockS3Send, mockCompose } = vi.hoisted(() => ({
  mockExecute: vi.fn(),
  mockCommit: vi.fn(),
  mockRollback: vi.fn(),
...
function wire(fx: Fx) {
  S3_JSON[`tenants/${T}/gen/${RUN}/${HKEY}.json`] = {
    harmonizationKey: HKEY, kind: 'prose',
    sentences: [{ text: 'The organization determines its context.', factRefs: ['F1'] }],
  };
  S3_JSON[`tenants/${T}/gen/${RUN}/leadership.json`] = {
    harmonizationKey: 'leadership', kind: 'prose',
    sentences: [{ text: 'Top management leads.', factRefs: ['F1'] }],
  };
  S3_JSON[`tenants/${T}/documents/${MASTER}/v1.json`] = {
    kind: 'master_list',
    entries: [
      { documentId: MANUAL, title: 'IMS Manual', docType: 'manual', standard: 'IMS', clauseRefs: ['4.1', '5.1'], status: 'draft', versionNo: 1, contentRef: `tenants/${T}/documents/${MANUAL}/v1.json` },
      { documentId: CLAUSE_DOC, title: 'Context', docType: 'procedure', standard: 'ISO9001', clauseRefs: ['4.1'], status: 'draft', versionNo: 1, contentRef: `tenants/${T}/documents/${CLAUSE_DOC}/v1.json` },
      { documentId: MATRIX, title: 'Matrix', docType: 'correlation_matrix', standard: 'IMS', clauseRefs: ['4.1', '5.1'], status: 'draft', versionNo: 1, contentRef: `tenants/${T}/documents/${MATRIX}/v1.json` },
    ],
  };
  ...
  mockExecute.mockImplementation(async (sql: string) => {
    if (sql.includes('FROM qms.generation_runs gr')) {
      ...
      return rows(
        ...
        [[['ISO9001'], fx.manualDocId, fx.runStatus,
          JSON.stringify({ legalName: 'Test Org', documentLocale: 'en', standardsInScope: ['ISO9001'] })]],
      );
    }
    ...
    if (sql.includes('FROM qms.generation_sections WHERE run_id')) {
      return rows(
        ...
        [
          [SECTION_ID, HKEY, fx.sectionKindDb, fx.sectionKindDb === 'failed' ? null : `tenants/${T}/gen/${RUN}/${HKEY}.json`, ['c-41']],
          ['sec-other', 'leadership', fx.otherFailed ? 'failed' : 'prose', fx.otherFailed ? null : `tenants/${T}/gen/${RUN}/leadership.json`, ['c-51']],
        ],
      );
    }
    ...
    if (sql.includes('status AS kind')) {
      return rows(
        ['id', 'kind', 'harmonization_key', 'clause_refs', 'content_sha256', 'reviewed_by', 'reviewed_at', 'error'],
...
describe('regenerateSection guards', () => {
  it('RUN_NOT_FOUND when the run does not exist', async () => {
    wire({ runStatus: 'MISSING', manualDocId: null, sectionKindDb: 'prose', clauseDocMatches: true, otherFailed: false });
  ...
  it('RUN_NOT_FINALIZED when manual_document_id is NULL (no doc plane to version)', async () => {
    wire({ runStatus: 'running', manualDocId: null, sectionKindDb: 'prose', clauseDocMatches: true, otherFailed: false });
    await expect(handler(input)).rejects.toThrow('RUN_NOT_FINALIZED');
    expect(mockCompose).not.toHaveBeenCalled();
  });

  it('SECTION_NOT_FOUND for an unknown harmonization key', async () => {
    wire({ runStatus: 'complete', manualDocId: MANUAL, sectionKindDb: 'MISSING', clauseDocMatches: true, otherFailed: false });
...
describe('regenerateSection happy path (prose → prose)', () => {
  it('resets review state, composes, writes manual v2 + clause-doc v2 + refreshed master list; no run-status change', async () => {
    wire({ runStatus: 'complete', manualDocId: MANUAL, sectionKindDb: 'prose', clauseDocMatches: true, otherFailed: false });

    const result = await handler(input) as Record<string, unknown>;

    // APR-1: review state cleared on reset
    const reset = mockExecute.mock.calls.find(c => (c[0] as string).includes("SET status = 'pending'"))!;
    ...
    expect(mockCompose).toHaveBeenCalledWith({ runId: RUN, tenantId: T, sectionId: SECTION_ID, sectionKey: HKEY });

    // Version INSERTs: manual v2 + clause v2 + master v2 (matrix unchanged — kind stable)
    const versionInserts = mockExecute.mock.calls.filter(c => (c[0] as string).includes('INSERT INTO m1.document_versions'));
    const insertedDocs = versionInserts.map(c => {
      const params = Object.fromEntries((c[1] as Array<{ name: string; value: Record<string, unknown> }>)
    ...
    const manualPut = mockS3Send.mock.calls.find(c =>
      c[0].constructor.name === 'PutObjectCommand' && c[0].input.Key === `tenants/${T}/documents/${MANUAL}/v2.json`)!;
    const manualBody = JSON.parse(manualPut[0].input.Body);
    expect(manualBody.sections.map((s: { harmonizationKey: string }) => s.harmonizationKey).sort())
      .toEqual([HKEY, 'leadership'].sort());

    // Master-list refresh: entries re-pointed at latest versions (Task-9 carry-forward closed)
    const masterPut = mockS3Send.mock.calls.find(c =>
      c[0].constructor.name === 'PutObjectCommand' && c[0].input.Key === `tenants/${T}/documents/${MASTER}/v2.json`)!;
    const masterBody = JSON.parse(masterPut[0].input.Body);
    const manualEntry = masterBody.entries.find((e: { documentId: string }) => e.documentId === MANUAL);
    ...
    expect(mockExecute.mock.calls.some(c => (c[0] as string).includes('UPDATE qms.generation_runs'))).toBe(false);
    ...
    expect(mockPublishAudit.mock.calls[0][0]).toMatchObject({ detailType: 'Generation.SectionRegenerated' });
    expect(mockCommit.mock.invocationCallOrder.at(-1)!)
...
describe('regenerateSection failure + repair semantics', () => {
  it('still-failed section ships NO clause-doc version; run flips complete → partial', async () => {
    ...
    wire({ runStatus: 'complete', manualDocId: MANUAL, sectionKindDb: 'failed', clauseDocMatches: true, otherFailed: false });

    await handler(input);

    const versionInserts = mockExecute.mock.calls.filter(c => (c[0] as string).includes('INSERT INTO m1.document_versions'));
    const docs = versionInserts.map(c => (c[1] as Array<{ name: string; value: { stringValue?: string } }>)
      .find(p => p.name === 'docId')!.value.stringValue);
    ...
    const statusUpdate = mockExecute.mock.calls.find(c => (c[0] as string).includes('UPDATE qms.generation_runs'))!;
    const params = Object.fromEntries((statusUpdate[1] as Array<{ name: string; value: Record<string, unknown> }>)
      .map(p => [p.name, Object.values(p.value)[0]]));
  ...
  it('previously-failed → prose CREATES the missing clause document and adds it to the master list', async () => {
    wire({ runStatus: 'partial', manualDocId: MANUAL, sectionKindDb: 'prose', clauseDocMatches: false, otherFailed: false });

    await handler(input);

    // New document inserted (no candidate matched the harmonization key)
    const docInsert = mockExecute.mock.calls.find(c => (c[0] as string).includes('INSERT INTO m1.documents'))!;
    ...
    const masterPut = mockS3Send.mock.calls.find(c =>
      c[0].constructor.name === 'PutObjectCommand' && c[0].input.Key === `tenants/${T}/documents/${MASTER}/v2.json`)!;
    const masterBody = JSON.parse(masterPut[0].input.Body);
    expect(masterBody.entries.some((e: { documentId: string }) => e.documentId === 'doc-new-created')).toBe(true);
