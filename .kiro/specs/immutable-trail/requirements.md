...
| CON-3 | **When** `appendAuditEvent` throws a `ConditionalCheckFailedException` (duplicate / replay), **the consumer shall** log the replay at WARN level and treat the message as successfully processed (no retry, no DLQ). Idempotent replays are safe because the SK ulid matches. |
...
| CON-6 | **[REV-3] The consumer shall** use `createHandler` in FIFO mode. In FIFO mode, on the first processing failure in a batch, the handler reports that record AND ALL SUBSEQUENT records in the batch as unprocessed (`batchItemFailures`). This preserves per-tenant ordering — a retried message must not chain AFTER its successors. `ConditionalCheckFailedException` (CON-3) is NOT a failure for this purpose (it is a successful idempotent replay). |
| CON-7 | **[REV-3] The FIFO-mode extension to `services/eventing` `createHandler`** shall be unit-tested in `services/eventing/__tests__/consumer.test.ts` with a test proving: given a 5-message batch where message #2 fails transiently, the returned `batchItemFailures` contains message #2, #3, #4, and #5. |
