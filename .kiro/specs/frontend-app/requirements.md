...
| BC-7 | **Consolidated new schema surface** (reviewed as ONE REQUIRES-HUMAN item at design): `listPendingHitlItems` (query, GSI9-backed), `resolveHitlItem` (mutation — this spec designs the contract), `onHitlItemResolved` (subscription). No other new schema surfaces without REQUIRES-HUMAN review. |
