...
- [x] **Task 1.1:** FrontendStack — add ContentDeployRole with deterministic
...
- [x] **Task 1.2:** ApiStack — capture existing `GraphqlApiUrl` CfnOutput
  into `public readonly graphqlApiUrlOutput: cdk.CfnOutput`. No logical ID
  change. Evidence: `tsc --noEmit` clean.

- [x] **Task 1.3:** IdentityStack — capture existing `PoolBId` and
...
- [x] **Task 1.4:** CumplifyStage — add six public fields and wire from the
  three stacks (§2.3). Evidence: `tsc --noEmit` clean.

## Phase 2 — Pipeline step + ordering

- [x] **Task 2.1:** pipeline-stack.ts — add `makeDeployFrontendStep` factory
...
- [x] **Task 3.1:** `frontend/scripts/load-env.mjs` — add early-exit guard
...
- [x] **Task 4.1:** `infra/lib/frontend-stack.unit.test.ts` — extend with
...
- [x] **Task 4.2:** `infra/lib/pipeline-stack.unit.test.ts` — extend with
...
- [x] **Task 4.3:** `frontend/src/test/load-env.test.ts` — extend with
...
- [x] **Task 5.1:** Run full gate: `tsc --noEmit` + `npm run test` (both
