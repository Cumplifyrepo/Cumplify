...
| **BC-10** | **EvidenceVault Object-Lock must be parameterized BEFORE any sealing task runs.** `data-stack.ts:379-388` hardcodes `COMPLIANCE / 2555 days` with `RemovalPolicy.RETAIN`. One sealed PDF in dev makes that bucket undeletable for 7 years, and makes any tenant retention policy under 7 years unenforceable. Mirror `auditArchiveRetentionDays` (env-parameterized, dev=1). [REQUIRES-HUMAN] |
