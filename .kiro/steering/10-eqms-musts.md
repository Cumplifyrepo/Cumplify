...
| DC-2 | Full version control with change summaries; prior versions retained, never silently lost | Diff any two versions; supersede chain intact |
| DC-3 | Configurable approval workflows: sequential, parallel, conditional by doc type; auto-escalation on overdue | Route matrix test incl. escalation timer firing |
...
| DC-6 | Controlled print/copy: stamped exports ("uncontrolled when printed", doc ID/version), on-demand complete copies for auditors | Export carries stamps + fingerprint |
...
| ES-4 | Secure, computer-generated, time-stamped audit trail of create/modify/delete, attributed per user, retained ≥ record retention, protected from alteration including by admins | Append-only IAM + hash chain + Object Lock COMPLIANCE |
...
- Training linkage: competence records (M13) bound to document revisions
  (DC-4). System flags untrained-on-current-revision — this is a **launch
  requirement, not roadmap** (the line between an eQMS and a shared drive).
- Availability: mobile access for Worker roles + offline-tolerant forms.
- Validation pack (Enterprise): vendor-maintained IQ/OQ/PQ-style evidence
  (requirements traceability, test evidence per release, change-control log)
  so regulated customers can adopt without self-validating from scratch.
